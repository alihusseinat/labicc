...
void imprimevalor(IDX * index, int t);
...
char * dotidx(char * str, char * namevar){

    str = corrector(str);

    int i;

    int T = strlen(str) + strlen(namevar) + 1;
    int t = strlen(str);
...
void dumpindex(SCHEMAO * schema, char * namevar){
    ...
    char * path = dotidx(schema->name, namevar);

    //printf("printando arquivo de nome -%s-\n", path);
...
void swapstruct(IDX * vet, int pos1, int pos2){
    ...
    vet[pos1].end = vet[pos2].end;
    vet[pos1].val = vet[pos2].val;

    vet[pos2].end = aux.end;
    vet[pos2].val = aux.val;

}
void combsort(IDX * vet, int t, int (*compare)(IDX *, IDX *)){ //algoritmo de ordencao combsort, é um algoritmo de troca, é parecido com o buble sort mas tem algumas partcularidades que
...
void imprimearquivo(SCHEMAO * schema, IDX * index, int t, int varsize, char * namevar){
    ...
    while(i < t){
        ...
        fwrite(index[i].val, varsize, 1, fp);

        fwrite(&index[i].end, 4, 1, fp);
...
void createindex(SCHEMAO * schema, char * namevar){

    FILE* fp = fopen(dotdata(schema->name), "r+");

    //printf("imprimindo arquivo de nome -%s- \n", dotidx(schema->name, namevar));

    int i = 0, ret, nidx = 0, varpos, varsize;
    ...
    do{
        ...
        s = malloc(schema->st[i]->size);

        ret = fread(s, schema->st[i]->size, 1, fp);

        if(!strcmp(schema->st[i]->name, namevar)){

            ++nidx;

            index = (IDX*)realloc(index, nidx * sizeof(IDX));

            index[nidx-1].val = s;
            index[nidx-1].end = (nidx - 1) * schema->sizeperrow;
    ...
    switch(findtype(schema, namevar, &varpos)){
        ...
        case 'i':
            combsort(index, nidx, comparei);
            break;
        case 'd':
            combsort(index, nidx, compared);
            break;
        case 'c':
            combsort(index, nidx, comparec);
            break;

    }

    imprimearquivo(schema, index, nidx, varsize, namevar);
